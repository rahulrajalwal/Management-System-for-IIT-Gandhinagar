...
if (isset($_POST['save'])) {
    ...
    // Determine who is adding the student
    $userId = $_SESSION['userId'];
    $userRole = $_SESSION['userRole']; // Assuming you have a userRole in session which tells whether user is Admin or Teacher
    ...
    if ($userRole == 'Admin') {
        $addedByType = 'Admin';
        $addedBy = $_SESSION['emailAddress']; // Assuming admin's email is stored in session
    } elseif ($userRole == 'Teacher') {
        $addedByType = 'Teacher';
        $teacherQuery = "SELECT emailAddress FROM tblclassteacher WHERE Id = '$userId'";
        $teacherResult = $conn->query($teacherQuery);
        
        if ($teacherResult->num_rows > 0) {
            $teacher = $teacherResult->fetch_assoc();
            $addedBy = $teacher['emailAddress'];
        }
    }

    // Insert query
    $query = "INSERT INTO tblstudents (aadharNumber, surname, name, secondName, mobile, alternateMobile, email, currentAddress, village, permanentAddress, age, dob, gender, education, currentStatus, familyWorkingStatus, workExperience, livePhoto, idFront, idBack, deposited, dateCreated, AddedByType, AddedBy) 
              VALUES ('$aadharNumber', '$surname', '$name', '$secondName', '$mobile', '$alternateMobile', '$email', '$currentAddress', '$village', '$permanentAddress', '$age', '$dob', '$gender', '$education', '$currentStatus', '$familyWorkingStatus', '$workExperience', '$livePhoto', '$idFront', '$idBack', '$deposited', NOW(), '$AddedByType', '$AddedBy')";
